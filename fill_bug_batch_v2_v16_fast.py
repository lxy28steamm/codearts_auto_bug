...
# ================== 配置 ==================
BUG_TXT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output.txt")
...
WAIT_CLOSE_MS = 80         # 关闭旧面板后的等待
WAIT_PANEL_MS = 700        # 等待新下拉面板展开的最长时间
WAIT_SEARCH_MS = 350       # 输入搜索内容后等待筛选结果
WAIT_CLICK_MS = 150        # 点击选项后的等待
WAIT_CLEAR_MS = 180        # 点击“提出人”小叉后的等待
WAIT_RETRY_MS = 450        # 首次展开失败后的重试等待
...
def close_all_dropdowns(page):
    ...
    try:
        ...
        pass
    page.wait_for_timeout(WAIT_CLOSE_MS if FAST_DROPDOWN_MODE else 500)
